...
module.exports.join = catcher(async (req, res, next) => {
  ...
  if (password !== confirmPassword)
    return next(
      new _Error("Password and Confirm Password must be the same", 400)
    );
...
module.exports.login = catcher(async (req, res, next) => {
  ...
  if (!user)
    return next(
      new _Error("No user found with this email, please sign up", 404)
    );

  const isCorrectPassword = await user.correctPassword(password);

  if (!isCorrectPassword)
    return next(new _Error("Email or Password is incorrect", 401));
...
module.exports.send_reset_OTP = catcher(async (req, res, next) => {
  ...
  if (!user)
    return next(new _Error("User not found Please check your email.", 404));
